...
func (s *InvokeRenderer) RenderRuntimeEvent(writer http.ResponseWriter, request *http.Request) error {
	...
	renderInvokeHeaders(writer, invoke.ID, customerTraceID, invoke.ClientContext,
		cognitoIdentityJSON, invoke.InvokedFunctionArn, deadlineHeader, invoke.ContentType, invoke.TenantID)
...
func renderInvokeHeaders(writer http.ResponseWriter, invokeID string, customerTraceID string, clientContext string,
	cognitoIdentity string, invokedFunctionArn string, deadlineMs string, contentType string, tenantID interop.TenantID) {
	...
	setHeaderIfNotEmpty(headers, "Lambda-Runtime-Aws-Tenant-Id", string(tenantID))
	if contentType == "" {
