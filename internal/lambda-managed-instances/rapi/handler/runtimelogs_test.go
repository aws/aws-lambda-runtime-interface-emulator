...
func (s *mockSubscriptionAPI) Configure(passphrase string, addr netip.AddrPort) {
	s.Called(passphrase, addr)
}
...
func TestSuccessfulRuntimeLogsResponseProxy(t *testing.T) {
	agentName, reqBody, reqHeaders := "dummyName", []byte(`foobar`), map[string][]string{"Key": {"VAL1", "VAL2"}}
	respBody, respStatus, respHeaders := []byte(`barbaz`), http.StatusNotFound, map[string][]string{"K": {"V1", "V2"}}
...
func TestSuccessfulTelemetryAPIPutRequest(t *testing.T) {
	agentName, reqBody, reqHeaders := "extensionName", []byte(`foobar`), map[string][]string{"Key": {"VAL1", "VAL2"}}
	respBody, respStatus, respHeaders := []byte(`"OK"`), http.StatusOK, map[string][]string{"K": {"V1", "V2"}}
...
func TestNumberOfSubscribersWhenAnExtensionIsAlreadySubscribed(t *testing.T) {
	agentName, reqBody, reqHeaders := "extensionName", []byte(`foobar`), map[string][]string{"Key": {"VAL1", "VAL2"}}
	respBody, respStatus, respHeaders := []byte(`"AlreadySubcribed"`), http.StatusOK, map[string][]string{"K": {"V1", "V2"}}
	numSubscribersMetric := telemetry.NumSubscribers
	subscribeSuccessMetric := telemetry.SubscribeSuccess
...
func TestErrorUnregisteredAgentID(t *testing.T) {
	...
	reqBody, reqHeaders := []byte(`foobar`), map[string][]string{"Key": {"VAL1", "VAL2"}}
	clientErrMetric := telemetry.SubscribeClientErr
	...
	request = request.WithContext(context.WithValue(context.Background(), model.AgentIDCtxKey, invalidAgentID))
	responseRecorder := httptest.NewRecorder()
	...
	expectedHeaders := http.Header(map[string][]string{"Content-Type": {"application/json"}})
